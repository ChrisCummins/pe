...
struct demo {
	...
	struct particle_emitter *engine[20];
...
static gboolean deactivate_firework(gpointer data)
{
	...
	demo->engine[demo->last_active]->source_active = FALSE;
...
static void ignite_firework(struct demo *demo, int i) {
	...
	demo->engine[i]->new_particles_per_ms = g_random_int_range(3000, 20000);
	demo->engine[i]->particle_size = g_random_double_range(2, 4);

	/* Position */
	demo->engine[i]->particle_position.value[0] = g_random_double_range(0, WIDTH);
	demo->engine[i]->particle_position.value[1] = g_random_double_range(0, HEIGHT / 2);
	...
	demo->engine[i]->particle_direction.variance[1] = 1.0f;
	demo->engine[i]->particle_direction.variance[2] = 1.0f;
	...
	/* Color */
	demo->engine[i]->particle_color.hue.value = g_random_double_range(0, 360.0f);
	demo->engine[i]->particle_color.hue.variance = g_random_double_range(0, 180.0f);
	...
	demo->engine[i]->particle_color.luminance.value =
		g_random_double_range(0.5, 0.9);
	demo->engine[i]->particle_color.luminance.variance = 0.1f;
	demo->engine[i]->particle_color.luminance.type = FLOAT_VARIANCE_PROPORTIONAL;

	demo->engine[i]->source_active = TRUE;
